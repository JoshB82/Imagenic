...
namespace _3D_Engine.Entities.SceneObjects.Meshes.TwoDimensions
{
    ...
    public sealed class Circle : Mesh
    {
        ...
        private void GenerateFaces()
        {
            if (Textures is null)
            {
                Triangles = new SolidTriangle[resolution];
                for (int i = 0; i < resolution - 1; i++) Triangles[i] = new SolidTriangle(Vertices[i + 1], Vertices[0], Vertices[i + 2]);
                Triangles[resolution - 1] = new SolidTriangle(Vertices[resolution], Vertices[0], Vertices[1]);
            }
            else
            {
                Triangles = new TextureTriangle[resolution];
                for (int i = 0; i < resolution - 1; i++) Triangles[i] = new TextureTriangle(Vertices[i + 1], Vertices[0], Vertices[i + 2], Textures[0].Vertices[i + 1], Textures[0].Vertices[0], Textures[0].Vertices[i + 2], Textures[0]);
                Triangles[resolution - 1] = new TextureTriangle(Vertices[resolution], Vertices[0], Vertices[1], Textures[0].Vertices[resolution], Textures[0].Vertices[0], Textures[0].Vertices[1], Textures[0]);
            }
        }
