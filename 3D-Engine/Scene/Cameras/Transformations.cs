...
namespace _3D_Engine
{
    public abstract partial class Camera : Scene_Object
    {
        ...
        public void Pan_Forward(double distance) => Translate(World_Direction * distance);
        ...
        public void Pan_Back(double distance) => Translate(World_Direction * -distance);
        ...
        public void Rotate_Up(double angle)
        {
            ...
            Set_Camera_Direction_1(new Vector3D(transformation_up * new Vector4D(World_Direction)), new Vector3D(transformation_up * new Vector4D(World_Direction_Up)));
        }
        public void Rotate_Left(double angle)
        {
            Matrix4x4 transformation_left = Transform.Rotate(World_Direction_Up, angle);
            Set_Camera_Direction_3(new Vector3D(transformation_left * new Vector4D(World_Direction_Right)), new Vector3D(transformation_left * new Vector4D(World_Direction)));
        }
        public void Rotate_Right(double angle)
        {
            Matrix4x4 transformation_right = Transform.Rotate(World_Direction_Up, -angle);
            Set_Camera_Direction_3(new Vector3D(transformation_right * new Vector4D(World_Direction_Right)), new Vector3D(transformation_right * new Vector4D(World_Direction)));
        }
        public void Rotate_Down(double angle)
        {
            Matrix4x4 transformation_down = Transform.Rotate(World_Direction_Right, -angle);
            Set_Camera_Direction_1(new Vector3D(transformation_down * new Vector4D(World_Direction)), new Vector3D(transformation_down * new Vector4D(World_Direction_Up)));
        }
        public void Roll_Left(double angle)
        {
            Matrix4x4 transformation_roll_left = Transform.Rotate(World_Direction, -angle);
        ...
        {
            Matrix4x4 transformation_roll_right = Transform.Rotate(World_Direction, angle);
