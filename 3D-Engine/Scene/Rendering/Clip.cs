...
namespace _3D_Engine
{
    public sealed partial class Scene
    {
        ...
        {
            Vector3D point_1 = new Vector3D(e.P1.Point), point_2 = new Vector3D(e.P2.Point);
            ...
            if (point_1_distance >= 0 && point_2_distance < 0)
            {
                ...
                Vector3D intersection = Vector3D.Line_Intersect_Plane(point_1, point_2, plane_point, plane_normal, out double d);
                e.P1 = e.P1;
                e.P2 = new Vertex(new Vector4D(intersection));
                return true;
            }
            if (point_1_distance < 0 && point_2_distance >= 0)
            {
                // One point is on the outside, the other on the inside, so clip the line
                Vector3D intersection = Vector3D.Line_Intersect_Plane(point_2, point_1, plane_point, plane_normal, out double d);
                e.P1 = e.P2;
                e.P2 = new Vertex(new Vector4D(intersection));
                return true;
            }
        ...
        {
            Vector3D point_1 = new Vector3D(f.P1.Point), point_2 = new Vector3D(f.P2.Point), point_3 = new Vector3D(f.P3.Point);
            ...
            switch (inside_point_count)
            {
                ...
                case 1:
                    ...
                    {
                        f1 = new Face(new Vertex(new Vector4D(inside_points[0])), new Vertex(new Vector4D(first_intersection)), new Vertex(new Vector4D(second_intersection)));
                    }
                    else
                    {
                        ...
                        f1 = new Face(new Vertex(new Vector4D(inside_points[0])), new Vertex(new Vector4D(first_intersection)), new Vertex(new Vector4D(second_intersection)), inside_texture_points[0], t_intersection_1, t_intersection_2, f.Texture_Object);
                    }
                    f2 = null;
                    return 1;
                case 2:
                    ...
                    if (f.Texture_Object == null)
                    {
                        f1 = new Face(new Vertex(new Vector4D(inside_points[0])), new Vertex(new Vector4D(inside_points[1])), new Vertex(new Vector4D(first_intersection)));
                        f2 = new Face(new Vertex(new Vector4D(inside_points[1])), new Vertex(new Vector4D(second_intersection)), new Vertex(new Vector4D(first_intersection)));
                    }
                    else
                    {
                        Vector3D t_intersection_1 = (outside_texture_points[0] - inside_texture_points[0]) * d1 + inside_texture_points[0];
                        Vector3D t_intersection_2 = (outside_texture_points[0] - inside_texture_points[1]) * d2 + inside_texture_points[1];
                        f1 = new Face(new Vertex(new Vector4D(inside_points[0])), new Vertex(new Vector4D(inside_points[1])), new Vertex(new Vector4D(first_intersection)), inside_texture_points[0], inside_texture_points[1], t_intersection_1, f.Texture_Object);
                        f2 = new Face(new Vertex(new Vector4D(inside_points[1])), new Vertex(new Vector4D(first_intersection)), new Vertex(new Vector4D(second_intersection)), inside_texture_points[1], t_intersection_1, t_intersection_2, f.Texture_Object);
                    }
                    return 2;
                case 3:
